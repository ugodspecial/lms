...
| §92 step | Content |
|---|---|
| **Architecture** | Four layers established. `Identity` + `Administration` + `Communication` domains. Fortify for password/2FA/verification/reset (headless, so the Blade/Livewire UI stays ours). spatie/laravel-permission v8 for RBAC. Policy registration in `AuthServiceProvider` with the Super-Admin `Gate::before`. Middleware: `AuthenticateArea`, `EnsurePermission`, `SetUserTimezone`, `SecurityHeaders`. Design system (`ui.*` Blade components). Audit + settings + files services |
| **Database changes** | 21 tables: `users`, `password_reset_tokens`, `sessions`, `cache`, `cache_locks`, `jobs`, `job_batches`, `failed_jobs`, `notifications`, `personal_access_tokens`, `roles`, `permissions`, `model_has_roles`, `model_has_permissions`, `role_has_permissions`, `connected_accounts`, `consents`, `settings`, `audit_logs`, `files`, `notification_preferences` |
| **Models** | `User`, `ConnectedAccount`, `Consent` (Identity) · `Setting`, `AuditLog`, `File` (Administration) · `NotificationPreference` (Communication) · spatie's `Role`, `Permission`. All under `app/Domain/<Domain>/Models` per docs/01 §5 — the skeleton's `app/Models` is retired, which is why `User` carries `#[UseFactory]`: the framework's factory guesser only strips a leading `App\Models\`. |
...
| 2026-09-08 | 1 | Phase 1 has three domains, not two: `Communication` is created for `NotificationPreference` | A consequence of the row above. `notification_preferences` is a communication record — it decides whether a notification is delivered on a channel — and putting its model in `Identity` or `Administration` would mean the first table of Phase 5 arriving in a domain that does not own it. The domain is created now with one model in it rather than having the model moved later. |
